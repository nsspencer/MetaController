...
class MetaController(type):
    ...
    def generate_action(cls: "MetaController"):
        # Create the nodes for the function arguments
        args = ast.Name(id="args", ctx=ast.Load())
        kwargs = ast.Name(id="kwargs", ctx=ast.Load())

        # Create the node for the *args unpacking
        starred_args = ast.Starred(value=args, ctx=ast.Load())

        # Create the node for the **kwargs unpacking
        kwargs_unpacking = ast.keyword(arg=None, value=kwargs)

        # Create the node for the function call
        func_call = ast.Call(
            func=ast.Name(id={ACTION_FN_NAME}, ctx=ast.Load()),
            args=[starred_args],
            keywords=[kwargs_unpacking],
        )

        # Create the node for the expression
        expr = ast.Expr(value=func_call)

        # Create the node for the module
